...
export const AboutSectionTwo: React.FC<AboutSectionTwoProps> = ({ 
  ...
  image 
    ...
    return (
        <motion.div
            className="sticky top-20 w-full mx-auto bg-white rounded-3xl p-10 my-16 border border-gray-200 shadow-xl flex flex-col md:flex-row gap-10 z-10"
            initial={{ scale: 1, opacity: 1, y: 40 }}
            whileInView={{ scale: 1, opacity: 1, y: 0 }}
            transition={{ duration: 0.8, ease: 'easeOut' }}
        >
            <div className="flex-1 min-w-0 md:pr-10">
                <h1 className="text-2xl font-bold mb-3 text-gray-900">{subtitle}</h1>
            ...
            <div className="flex-1 flex justify-center items-center mt-10 md:mt-0">
                <div className="w-80 h-80 rounded-full overflow-hidden shadow-xl border-4 border-white flex items-center justify-center bg-gray-100">
                    <img
                        src={image}
                        alt={title}
